...
const Toast = ({ isShown, message, type, onClose }) => {
  ...
  return (
    <div className={`absolute top-20 right-6 transition-all duration-400 ${isShown ? "opacity-100" : "opacity-0"
      ...
      <div className={`min-w-32 bg-white border shadow-2xl rounded-md after:w-[5px] after:h-full ${type === "delete" ? "after:bg-red-500" : "after:bg-green-500"
        ...
        <div className="flex items-center gap-3 py-2 px-4">
          ...
          <p className="text-sm text-slate-8000">{message}</p>
        </div>
      </div>
    </div>
  )
